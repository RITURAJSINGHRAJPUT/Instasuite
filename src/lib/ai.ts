...
const REPLY_GUARD = [
  ...
  "Once you have FINALIZED a reservation or takeaway earlier in this conversation (you confirmed it back to the guest and/or emitted its hand-off line), treat any LATER message as a brand-new request: greet and help the guest afresh, and do NOT resume, re-confirm, or re-emit the hand-off for that finished order. Only revisit a past order if the guest explicitly asks about it (to check or change it). You may still reuse their name, contact and preferences, and only start a new hand-off line if they actually place a new order.",
].join("\n");
